# split ASV table, tree and taxonomy by region and place in separate folders respectively. Each file called Asv_table.txt, tree.nwk and classification.txt, respectively
# the code will iterate over these folders and perform iCAMP for each region

dirs<-list.dirs(full.names=F, recursive = FALSE)

for(i in dirs){
  wd=paste0("path_to_directory",i)   ### change to directory
  com.file="Asv_table.txt"    # the ASV table file (Tab delimited txt file)
  tree.file="tree.nwk"        # the phylogenetic tree file
  clas.file="classification.txt"  # the classification (taxonomy) information
  
  # the folder to save the output. please change to a new folder even if you are just testing the example data.
  save.wd=paste0(wd,"/out2") 
  ...
  nworker=48 # nworker is thread number for parallel computing, which depends on the CPU core number of your computer.
